...
-(void) getServerConnect
{
    ...
    sockAddr.sin_port = htons(1512);     // 포트 설정
    sockAddr.sin_family = AF_INET;
    sockAddr.sin_addr.s_addr =inet_addr("127.0.0.1");    // 접속할 서버 IP 설정
...
-(void)ReceiveReqvCommand:(NSString *)strMessage
{
    ...
    NSString *tId = [[messageArr objectAtIndex:1] substringToIndex:[[messageArr objectAtIndex:1] rangeOfString:@"\r\n"].location];
        ...
        //  pChatTargetIndex = [self searchUserID:tId];  //회원검색
...
-(void)startChart
{
    pStatus = CHAT;
        ...
        //[self.presentingViewController ChatViewShow];
...
-(void) setMemberInfomation:(NSArray *)memberInfo
{
    ...
    pChatData.pUserID = [memberInfo objectAtIndex:0];      // 회원 아이디
    pChatData.pUserName = [memberInfo objectAtIndex:1];    // 회원 이름
    
    pChatData.pContext = @"안녕하세요";
...
-(void)addChatMessage:(NSString *)strMessage DisTime:(NSString *) time forDirection:(bool)derection ReLoadData:(bool)forRefresh
{
    ...
    if(forRefresh == true) [pChatViewController.pChatListView reloadData];
}
